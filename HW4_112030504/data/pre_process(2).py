...
def select_imaegs(image_paths, images_num=200):
    ...
    sorted_images = sorted(image_paths, key=calculate_brightness)
    selected_image_paths = sorted_images[:images_num]
    """
    if images_num > len(image_paths):
        raise ValueError("Number of images to select exceeds the total number of images available.")

    selected_image_paths = random.sample(image_paths, images_num)
